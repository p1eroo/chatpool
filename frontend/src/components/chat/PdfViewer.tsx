...
export function PdfViewer() {
  const pdfViewerMessageId = useUIStore((s) => s.pdfViewerMessageId);
  ...
  const pdfMessages = useMemo(() => {
    ...
    return messages.filter((message) => {
      ...
      return isPdfFile(fileName) && (message.fileUrl || message.attachmentUrl);
    });
  ...
  const currentMessage =
    currentIndex >= 0 ? pdfMessages[currentIndex] : null;
  ...
  const handlePrint = () => {
    ...
    iframe.onerror = () => {
      showToast("No se pudo imprimir el documento");
      cleanup();
    };
  ...
  return createPortal(
    <div
      data-modal-overlay
      className="fixed inset-0 z-[200] bg-[#2a2a2a] animate-fade-in"
    >
      <div className="absolute inset-x-0 top-0 z-20 flex h-14 items-center border-b border-white/8 bg-[#1f1f1f]/95 px-4 md:px-5">
        <p className="min-w-0 flex-1 truncate pr-4 text-sm font-medium text-white/90">
          {fileName}
        </p>
