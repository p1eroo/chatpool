...
/** Ancho máximo cómodo de lectura (~hoja A4/carta ligeramente ampliada). */
...
export async function getPdfFitScale(
  ...
  const availableWidth = Math.min(
    Math.max(containerWidth - 96, 320),
    STANDARD_MAX_WIDTH
  );

  const fitScale =
    viewport.width <= availableWidth ? 1 : availableWidth / viewport.width;
